...
class PublicationAddNewGeneralSettingsForm extends UI\Form {
    ...
    public function __construct(IContainer $parent = NULL, $name = NULL) {
        ...
        $this->addText('spring_token', 'Spring token')->addRule($this::MAX_LENGTH, 'Spring token is way too long', 250)->setRequired('Spring token is required.');
        $this->addText('deadline_notification_advance', 'Conference deadlines notification advance [days]', $items)
            ->setRequired(true)->addRule(self::INTEGER, 'The deadline advance must be a number')
            ->addRule(self::MIN, 'The deadline advance must be min. 1 day', 1)->addRule(self::MAX, 'The deadline must be max. 90 days', 90);
